...
function Manager(options) {
    ...
    if (!(name in this.types))
        this.types[name] = { name: name, data: names, type: 'selection' }
...
Manager.prototype.startup = function startup(data) {
  ...
  if (plugin[this.endpoint]) {
    this.plugins[plugin.name] = plugin
    this.names.push(plugin.name)
  }
}

Manager.prototype.shutdown = function shutdown(plugin) {
  var plugin = data.plugin
  if (plugin[this.endpoint]) {
...
Manager.prototype.plug = function plug(data) {
    ...
    env.on("plugin:startup", this.startup)
    env.on("plugin:shutdown", this.shutdown)
    Object.keys(plugins).forEach(function(name) {
        this.startup({ env: env, plugin: plugins[name] })
    }, this)
}
Manager.prototype.unplug = function unplug(data) {
    ...
    env.removeListener("plugin:startup", this.startup)
    env.removeListener("plugin:shutdown", this.shutdown)
    Object.keys(plugins).forEach(function(name) {
        this.shutdown({ env: env, plugin: plugins[name] })
    })
}
