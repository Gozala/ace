...
function onPluginStartup(event) {
    var plugin = event.plugin, settings = plugin.settings;
...
function onTypeInstall(event) {
    var type = event.descriptor;
}
function onTypeUninstall(event) {
    var type = event.descriptor;
}
...
function register(env, descriptor, name) {
    ...
    env._emit('setting:install', {
        env: env,
        settings: settings,
        setting: Setting(descriptor, name)
    });
}
...
exports.startup = function startup(data) {
    ...
    env.on("setting:reset", onReset);

    env.on("type:install", onTypeInstall);
    env.on("type:uninstall", onTypeUninstall);

    // Register listener to register all the plug-ins that will be plugged.
    env.on("plugin:startup", onPluginStartup);

    // Register listener to unregister all the plug-ins that will be unplugged.
    env.on("plugin:shutdown", onPluginShutdown);

    // Go through all plugged plug-ins and register them.
    Object.keys(plugins).forEach(function (name) {
        onPluginStartup({ env: env, plugin: plugins[name] });
    });
};
...
exports.shutdown = function shutdown(data) {
    ...
    env.removeListener("setting:reset", onReset);
    env.removeListener("plugin:startup", onPluginStartup);
    env.removeListener("plugin:shutdown", onPluginShutdown);
    env.removeListener("type:install", onTypeInstall);
    env.removeListener("type:uninstall", onTypeUninstall);
