...
function register(env, descriptor, name) {
    ...
    if (baseType || !base) {
        env._emit('type:install', {
            env: env,
            types: types,
            descriptor: type = Type(descriptor, baseType)
        })
...
exports.description = [
    ...
    '',
    '   - "type:register"       When new type is registered.',
    '   - "type:broken"         When type registered that depenends on other,',
